...
@section('title', 'Create Research Article')
...
@section('content')
    <div class="max-w-5xl mx-auto" x-data="{ 
                    ...
                    slug: '',
                    authors: [
                        { name: '', email: '', affiliation: '', is_corresponding: 0 }
                    ],
                    addAuthor() {
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-serif font-bold text-slate-800">Publish New <span
                        class="text-blue-600">Article</span></h1>
                <p class="text-slate-500 mt-1">Register a peer-reviewed manuscript into the scholarly database.</p>
            </div>
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 gap-8">
                        <div>
                            ...
                            <textarea name="title" id="title" x-model="title" rows="2" @input="generateSlug()"
                                ...
                                placeholder="Full title of the manuscript..." required>{{ old('title') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" x-model="slug" value="{{ old('slug') }}"
                                    class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-mono text-xs"
                                    readonly required>
                                @error('slug') <p class="text-red-500 text-xs mt-2 ml-1">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('doi') <p class="text-red-500 text-xs mt-2 ml-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8 bg-slate-50/50">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            @error('journal_id') <p class="text-red-500 text-xs mt-2 ml-1">{{ $message }}</p> @enderror
                        </div>
            ...
            {{-- Author Information Section (Premium Dynamic Module) --}}
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                    ...
                    <button type="button" @click="addAuthor()"
                        class="text-[10px] font-black uppercase tracking-widest bg-emerald-600 text-white px-4 py-2 rounded-full hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-600/20">
                        + Add Scholar
                    </button>
                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                <div class="md:col-span-2">
                                    <label
                                        ...
                                        Scholar Name</label>
                                    <input type="text" :name="`authors[${index}][name]`" x-model="author.name"
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    <input type="text" :name="`authors[${index}][affiliation]`" x-model="author.affiliation"
                                        placeholder="Department of Science, Tech University"
                                ...
                                <div class="flex items-end pb-1">
                                    <label class="flex items-center gap-3 cursor-pointer group/toggle">
                                        ...
                                        <span
                                            class="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover/toggle:text-slate-600">Corresponding
                                            Author</span>
                                    </label>
                                </div>
                            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                <div class="p-8 space-y-8">
                    <div>
                        ...
                        <textarea name="abstract" id="abstract" rows="4"
                            ...
                            placeholder="Summary of research findings...">{{ old('abstract') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                        <div>
                            ...
                            <input type="file" name="pdf_path" id="pdf_path" class="hidden"
                                ...
                                x-data="{ fileName: 'Select PDF Document' }">
                            <label for="pdf_path"
                ...
                <div
                    ...
                    <div class="flex items-center gap-6 px-10">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(59,130,246,0.5)]">
                            </div>
                            <span class="text-[10px] font-black uppercase tracking-[0.35em] text-blue-400/80">Metadata
                    ...
                    <div class="flex items-center pr-1.5 gap-2">
                        ...
                        <button type="submit"
                            ...
                            <span class="relative z-10 text-[11px] font-black uppercase tracking-[0.4em]">Commit
                                Manuscript</span>
                        </button>
                    </div>
                </div>
